            ...
            <tbody className="divide-y divide-slate-100">
              {stats?.recentEnrollments?.map((enrollment, index) => (
                <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                  <td className="py-4 px-6 text-slate-800 font-medium">User #{enrollment.userId}</td>
                  <td className="py-4 px-6 text-slate-600">{enrollment.skillGroupId}</td>
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      <div className="w-full bg-slate-100 rounded-full h-2 max-w-[100px]">
                        <div className="bg-[#0D9488] h-2 rounded-full" style={{ width: `${enrollment.progressPercent}%` }}></div>
                      </div>
                      <span className="text-xs text-slate-500">{enrollment.progressPercent}%</span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
